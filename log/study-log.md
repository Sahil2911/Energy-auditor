...
### Three corrections raised by the student — 06 Sep

She raised three, and all three were fair.

**1. Mock 1 was not all unseen. Four of eight questions had been assigned.**

| Slot | Previously assigned as |
|---|---|
| L-1 — 25th L-1 | **Day 1, Block B3** (verbatim) |
| N-4(A) — 20th N-3 | **Sector diagnostic Q2** |
| N-4(B) — 24th N-4(D) | **Day 13, Block C** |
| N-4(C) — 24th N-4(C) | **Day 13, Drill 68** |

All three N-4 options. Two had been written days earlier by the same hand that then
called them unseen — the reuse of N-4(A) was even recorded in a commit message. The
claim came from memory rather than a check.

She answered two of them, so **22 of ~84 marks came from seen questions.** On the
genuinely unseen material the score is **62/75 = 83%** against 84% overall — the
rate is unchanged and the verdict stands, but the headline was inflated by
construction. It also sharpens the N-4(C) units error: `day-13.md` prints "Drill 68:
**14.49 MW**", so she wrote 347.659 MW with the right answer already published to
her. That is a checking-pass failure on top of a units one.

**Guard built:** `question-bank/used-ledger.md` — 36 questions, auto-generated by
`tools/ledger.py` from every reference in `curriculum/` and `assessments/`. **No
question enters a paper without being looked up there.** A rule I hold myself to
mechanically rather than by memory, because memory is what failed.

**2. Seven questions left unclassified in the question bank.**

Resolved. They were never unclassifiable — the classifier keyed on opening words and
these opened mid-answer (`Ans Effy ᶯ = …`) or with unmatched wording. **Seven rows
are four distinct questions**, Green and Pink duplicating:

| Question | Actually is | Filed under |
|---|---|---|
| 17th L-2 | Tracer dilution in an open channel | Pumps, Day 8 — **done, 4/5** |
| 20th N-2 | Pressurised hot water circulation | Boilers, Day 3 — **not yet done** |
| 25th L-1 | LP/HP boilers, ER 14 | Boilers, Day 1 — **done twice** |
| 25th N-4(D) | Textile stenter into a boiler duty | Textile, Day 13 — **not yet done** |

The two untouched ones are both worth doing. 20th N-2 is the same chain as the 19th
N-3 she scored 20/20 on, so it is a fair cold re-test; 25th N-4(D) uniquely joins
the stenter moisture balance to an evaporation ratio.

**3. When will the scanned papers be OCR'd?**

Fair question — it had been a "standing risk" with no date since Day 1. **The 21st
is done today.** PyMuPDF installed, pages rendered at 170 dpi and read directly;
every question and every model answer is legible. Full inventory in
`question-bank/21st-sitting.md`. The 22nd is next and becomes the source for Mock 2.

**Two findings from the 21st that land directly on current gaps:**

- **N-2 states the condenser duty formula in general form:**
  `Condenser duty = TR × 3024 × (1 + 1/COP)`. One expression for both machines; the
  multiplier is 1.227 at COP 4.4 and **1.833** at COP 1.2. Added to Day 15 Rung 163
  with a table — this is exactly the concept Mock 1's N-4(A) turned on.
- **N-4(A) is a hot-duct pitot correction with the static pressure printed as
